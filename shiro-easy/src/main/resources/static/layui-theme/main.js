layui.use(['element', 'form', 'layer', 'upload'], function () {
    ...
    /* 最大化窗口 */
    $(".timy-screen-full").on("click", function (e) {
        e.preventDefault();
    ...
    $(".ajax-status").on("click", function (e) {
        e.preventDefault();
        var checked = [];
        var tdcheckbox = $(".timy-table td .timy-checkbox :checkbox:checked");
    ...
    // 展示数据列表-多选框
    var thcheckbox = $(".timy-table th .timy-checkbox :checkbox");
    thcheckbox.on("change", function () {
        var tdcheckbox = $(".timy-table td .timy-checkbox :checkbox");
    ...
    // 检测列表数据是否为空
    var timyTable = $(".timy-table tbody");
    if (timyTable.length > 0) {
        var children = timyTable.children();
        if (children.length === 0) {
            var length = $(".timy-table thead th").length;
            var trNullInfo = "<tr><td class='timy-table-null' colspan='"
                + length + "'>没有找到匹配的记录</td></tr>";
            timyTable.append(trNullInfo);
        }
    }
    ...
    var paramSkip = function () {
        var getSearch = "";
        // 搜索框参数
        $('.timy-search-box [name]').each(function (key, val) {
    ...
    /* 展示列表数据搜索 */
    $(document).on("click", ".timy-search-btn", function () {
        paramSkip();
    });
    /* 改变显示页数 */
